...
def Extract_Features_Training(X):
    cv = Pipeline([('count', CountVectorizer(max_features = 5000, ngram_range=(1,2))),('tfid', TfidfTransformer())])
...
def Train_Model(Path):
    ...
    report={"Data Url:":"https://www.politifact.com"}
    report=model_eval(report, clf, X_test_text1, y_test_text1)
    # save the model to disk
    filename = 'Model4_SVC_Title.sav'
    ...
    with open('Model4_SVC_Report_Title.json', 'w', encoding='utf-8') as f:
...
def Predict(q):
    ...
    filename = path+'/Model4_SVC_Title.sav'
    print(filename)
    ...
    # Opening JSON file
    f = open(path+'/Model4_SVC_Report_Title.json') 
    report = json.load(f)
...
if __name__ == "__main__":
    ...
    Path="new_train_filter_16769.csv"
    ...
    # Opening JSON file
    f = open('Model4_SVC_Report_Title.json') 
    report = json.load(f)
